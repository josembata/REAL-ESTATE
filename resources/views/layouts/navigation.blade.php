<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    @if(Auth::user()->isCustomer())
        <x-nav-link :href="route('customer.favorites')" :active="request()->routeIs('customer.favorites')">
            {{ __('Saved Properties') }}
        </x-nav-link>
        <x-nav-link :href="route('customer.viewings')" :active="request()->routeIs('customer.viewings')">
            {{ __('Viewings') }}
        </x-nav-link>
    @endif
@endauth
                       ...
                       <x-slot name="trigger">
                            <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 focus:ring focus:ring-gray-200 transition">
                                <img class="h-20 w-15 rounded-full object-cover" 
                                     src="{{ Auth::user()->avatar ? asset(Auth::user()->avatar) : asset('default-avatar.png') }}" 
                                     alt="{{ Auth::user()->name }}" />
                            </button>
                        </x-slot>
